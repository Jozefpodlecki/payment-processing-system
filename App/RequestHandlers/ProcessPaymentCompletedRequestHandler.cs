...
{
    public class ProcessPaymentCompletedRequestHandler : IRequestHandler<ProcessPaymentCompletedRequest, bool>
    {
        ...
        public ProcessPaymentCompletedRequestHandler(
        ...
        public async Task<bool> Handle(ProcessPaymentCompletedRequest request, CancellationToken cancellationToken)
        {
            var payment = await _paymentRepository.GetByIdAsync(request.PaymentId, request.UserId, cancellationToken);
            
            payment.Status = PaymentStatus.Completed;
            payment.CompletedOn = _systemClock.UtcNow;
